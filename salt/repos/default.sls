...
{% if not grains.get('roles') or ('server' not in grains.get('roles') and 'suse_manager_proxy' not in grains.get('roles')) %}
...
tools_pool_repo_master:
  pkgrepo.managed:
    ...
    - priority: 98
{% endif %}
{% endif %} {# not grains.get('roles') or ('server' not in grains.get('roles') and 'suse_manager_proxy' not in grains.get('roles')) #}
