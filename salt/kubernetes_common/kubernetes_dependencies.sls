...
{% set storage_class = grains.get('kubernetes_storage_class') %}
{% set local_path_provisioner_path = grains.get('local_path_provisioner_path') %}
{% set scc_slmicro_pass = grains.get('scc_slmicro_pass') %}

{% set pkg_map = {
  'openSUSE Tumbleweed' : ['jq', 'checkpolicy', 'policycoreutils', 'container-selinux', 'openssl', 'git', 'iptables'],
  'Ubuntu': ['python3-yaml']
} %}
...
copy_local-path-storage_installation_file:
  file.managed:
    - name: {{ local_path_provisioner_file }}
...
variables_rke2:
  file.managed:
    ...
    - contents: |
        ...
        export LOCAL_PATH_PROVISIONER_PATH={{ local_path_provisioner_path }}
