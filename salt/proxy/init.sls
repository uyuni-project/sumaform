...
{% elif '4.3' in grains.get('product_version') %}
  {% if '-released' in grains.get('product_version') %}
    {% set client_tools_repo = grains.get("mirror") | default("dist.suse.de/ibs", true) ~ '/SUSE/Updates/SLE-Manager-Tools/15/x86_64/update/' %}
  {% else %}
  ...
  {% endif %}
{% else %}
    {% set client_tools_repo =  grains.get("mirror") | default("dist.suse.de", true) ~ '/ibs/Devel:/Galaxy:/Manager:/Main:/MLMTools-Beta-SLE15/images/repo/ManagerTools-Beta-SLE15-Pool-x86_64-Media1/' %}
{% endif %}
