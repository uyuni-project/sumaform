...
copy_value_yaml_file:
  file.managed:
    ...
    - source: salt://server_containerized/values.yaml
    - template: jinja
    - context:
        pass_product: admin
        pass_db: admin
...
copy_chart_yaml_file:
  file.managed:
    ...
    - source: salt://server_containerized/Chart.yaml
    - template: jinja
    - context:
...
# In principle we are not using Traefik as ingress controller, but it has dependencies in common and fails if isen't installed
install_traefik:
  cmd.run:
