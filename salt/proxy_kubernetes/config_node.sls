...
generate_proxy_config_file:
  ...
  - name: |
      ...
      ssh {{ grains['server'] }} "kubectl exec \$Server_pod -n \$Namespace --  spacecmd -u admin -p admin proxy_container_config -- pablo-proxy.mgr.suse.de pablo-server.mgr.suse.de 2048 galaxy-noise@suse.com ca.crt tls.crt tls.key"
...
copy_uyuni_ca:
  ...
  - name: |
      source {{ ProxyCertVarsFile }}
      ssh {{ grains['server'] }} "kubectl get cm -n $Namespace uyuni-ca "jsonpath={.data.ca\.crt}" >root-ca.crt"
