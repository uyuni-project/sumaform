...
register_suse_manager_server_with_scc:
   cmd.run:
     - name: SUSEConnect --url https://scc.suse.com -r {{ grains.get("proxy_registration_code") }} -p SUSE-Manager-Proxy/4.4/x86_64
add_sle_module_basesystem:
   cmd.run:
...
add_sle_module_suse_manager_server:
   cmd.run:
     - name: SUSEConnect -p sle-module-suse-manager-proxy/4.4/x86_64
{% endif %}
