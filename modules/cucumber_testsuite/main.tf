...
locals {
  ...
  kubernetes_external_cluster   = var.kubernetes && var.kubernetes_cluster_mode == "external"
  kubernetes_managed_cluster    = var.kubernetes && var.kubernetes_cluster_mode == "managed"
  ...
  server_configuration          = local.kubernetes_external_cluster ? local.external_server_configuration : (var.kubernetes ? module.server_kubernetes[0].configuration : ( var.container_server ? module.server_containerized[0].configuration : module.server[0].configuration))
  proxy_configuration           = local.kubernetes_external_cluster ? local.empty_proxy_configuration : (var.kubernetes ? module.proxy_kubernetes[0].configuration : (var.container_proxy ? module.proxy_containerized[0].configuration : module.proxy[0].configuration))
}
...
module "server_kubernetes" {
  ...
  count = local.kubernetes_managed_cluster ? 1 : 0
...
module "controller" {
  ...
  install_kubectl_helm     = var.install_kubectl_helm || local.kubernetes_external_cluster
  kubeconfig_path          = var.kubeconfig_path

  prometheus_push_gateway_url = var.prometheus_push_gateway_url

  install_kubernetes_server_on_external_cluster = local.kubernetes_external_cluster && var.install_mlm_server
...
output "configuration" {
  value = {
    ...
    proxy = local.proxy_configuration
