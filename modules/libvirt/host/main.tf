...
authorized_keys: [${var.ssh_key_path == "/dev/null" ? "${trimspace(file(var.base_configuration["ssh_key_path"]))}" : "${trimspace(file(var.base_configuration["ssh_key_path"]))}, ${trimspace(file(var.ssh_key_path))}"}]
reset_ids: true
${var.grains}
