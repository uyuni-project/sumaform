...
resource "aws_instance" "instance" {
  ami = "${var.provider_settings["ami"]}"
  instance_type = "${var.instance_type}"
  count = "${var.quantity}"
  availability_zone = "${var.provider_settings["availability_zone"]}"
  key_name = "${var.provider_settings["key_name"]}"
  subnet_id = "${var.provider_settings["private_subnet_id"]}"
  vpc_security_group_ids = ["${var.provider_settings["private_security_group_id"]}"]
...
resource "null_resource" "host_salt_configuration" {
  ...
  connection {
    ...
    private_key = "${file(var.provider_settings["key_file"])}"
    bastion_host = "${var.provider_settings["mirror_public_name"]}"
  }
...
use_avahi: False
mirror: ${var.provider_settings["mirror_private_name"]}
